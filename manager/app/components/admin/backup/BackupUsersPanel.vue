      ...
      <AdminTable :loading="loading">
        ...
        <template #mobile>
          ...
          <AdminMobileCard
            ...
            :title="String(row.nickname || row.phone || row.email || row.id)"
            :subtitle="String(row.id)"
          >
            <template #badge>
              <AdminBadge>{{ row.status }}</AdminBadge>
            </template>
            <AdminMobileMeta :label="$t('fields.phone')">
              {{ row.phone ?? $t("common.emDash") }}
            </AdminMobileMeta>
            <AdminMobileMeta :label="$t('fields.email')">
              {{ row.email ?? $t("common.emDash") }}
            </AdminMobileMeta>
